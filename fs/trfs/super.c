...
static void trfs_put_super(struct super_block *sb)
{
	...
	if (!spd)
		return;
	printk("\n SUPER -----------> vacant = %d", vacant);
/*	if(vacant != PAGE_SIZE){
		if(spd->out_fd){
			write_file_fd = spd->out_fd;
		...
		if(write_file_fd != NULL){
			...
			oldfs = get_fs();
			set_fs(KERNEL_DS);
			vfs_write(write_file_fd, write_file_buff, TRFS_PAGE_SIZE - vacant, &write_file_fd->f_pos);
			set_fs(oldfs);	
		}
		printk("\n write_file_buff ----> %s",write_file_buff);	
	}*/
	if(task)
                kthread_stop(task);
...
/*	if(trfs_record_struct){
		...
		kfree(trfs_record_struct); AAAAAAA
	...
	printk("\n TRFS_UMOUNT_BEGIN");
	if(spd->out_fd){
		filp_close(spd->out_fd, NULL);
		printk("\n FREEING TRFS_RECORD_STRUCT WHILE UNMOUNTING");
	}
	kfree(spd);
	sb->s_fs_info = NULL;
}
