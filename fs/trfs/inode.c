			 ...
			 umode_t mode, bool want_excl)
{
	...
	if (err)
		goto out;
	//AAAAAAAAAAAAAA
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_CREATE);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
	...
	/***********************************************************************************************************/
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(dentry->d_sb)->bitmap, TRFS_OP_CREATE)){

                trfs_record_struct->type = TRFS_OP_CREATE;
                trfs_append_to_list(dir, 0,trfs_record_struct); //TODO
                trfs_append_to_list(dentry, 1,trfs_record_struct);//TODO
...
out:
	...
	if(trfs_record_struct) //AAAAAAAAAAAAAAAAAAAAAAAAAAAA
		       ...
		       struct dentry *new_dentry)
{
	...
	/***********************************************************************************************************/
	//AAAAAAAAAAAAAAAA	
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_HARDLINK);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
        ...
        trfs_record_struct->data = NULL;*/
	//AAAAAAAAAAAAAAAA
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(old_dentry->d_sb)->bitmap, TRFS_OP_HARDLINK)){

                printk("\nTRFS_LINK ");
                trfs_record_struct->type = TRFS_OP_HARDLINK;
                trfs_append_to_list(dir, 0, trfs_record_struct);
                trfs_append_to_list(old_dentry, 1,trfs_record_struct);
		trfs_append_to_list(new_dentry, 1,trfs_record_struct);
		trfs_record_struct->success_flag = err;
                trfs_add_record(trfs_record_struct);
        }
	/***********************************************************************************************************/
...
static int trfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_UNLINK);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
	...
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(dentry->d_sb)->bitmap, TRFS_OP_UNLINK)){

                printk("\nTRFS_UNLINK ");
                trfs_record_struct->type = TRFS_OP_UNLINK;
                trfs_append_to_list(dir, 0,trfs_record_struct);
			  ...
			  const char *symname)
{
	...
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_SYMLINK);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
	...
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(dentry->d_sb)->bitmap, TRFS_OP_SYMLINK)){

                printk("\nTRFS SYMLINK");
		if(trfs_record_struct->data == NULL)
			trfs_record_struct->data = kmalloc(strlen(symname)*sizeof(char) +1, GFP_KERNEL);
		if(trfs_record_struct->data == NULL){
			err = -ENOMEM;
			goto out;
		}
		strcpy((char*)trfs_record_struct->data, symname);
		((char*)(trfs_record_struct->data))[strlen(symname)] = '\0';
                //printk("trfs_record_struct->data %s", trfs_record_struct->data);
		trfs_record_struct->type = TRFS_OP_SYMLINK;
                trfs_append_to_list(dir, 0,trfs_record_struct);
		//printk("\nAFTER INODE");
                trfs_append_to_list(dentry, 1,trfs_record_struct);
		trfs_record_struct->success_flag = err;
...
static int trfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	/***********************************************************************************************************/
	//AAAAAAAAAAAAAAAAAAAAAAA	
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_MKDIR);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
	...
	//AAAAAAAAAAAAAAAAAAAAAAAA
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(dentry->d_sb)->bitmap, TRFS_OP_MKDIR)){
		
		printk("\nTRFS MKDIR");
		trfs_record_struct->type = TRFS_OP_MKDIR;
		trfs_append_to_list(dir, 0,trfs_record_struct);
		trfs_append_to_list(dentry, 1,trfs_record_struct);	
		trfs_record_struct->success_flag = err;
		trfs_add_record(trfs_record_struct);
...
static int trfs_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_RMDIR);/*(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
	...
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(dentry->d_sb)->bitmap, TRFS_OP_RMDIR)){
                printk("\nTRFS_RMDIR ");
                trfs_record_struct->type = TRFS_OP_RMDIR;
                trfs_append_to_list(dir, 0,trfs_record_struct);
                trfs_append_to_list(dentry, 1,trfs_record_struct);
		trfs_record_struct->success_flag = err;
                trfs_add_record(trfs_record_struct);
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	/***********************************************************************************************************/
	//AAAAAAAAAAAAAAAAAAa
	trfs_record_struct = get_trfs_record_struct(TRFS_OP_RENAME);
/*	//(struct trfs_record_struct*)kmalloc(sizeof(struct trfs_record_struct), GFP_KERNEL);
        if(trfs_record_struct == NULL){
                err = -ENOMEM;
                goto out;
        }
...
*/
	//AAAAAAAAAAAAAAAA
	if(trfs_record_struct != NULL && is_enabled(TRFS_SB(old_dentry->d_sb)->bitmap, TRFS_OP_RENAME)){

                printk("\nTRFS_RENAME ");
                trfs_record_struct->type = TRFS_OP_RENAME;
		trfs_append_to_list(old_dir, 0,trfs_record_struct);
                trfs_append_to_list(new_dir, 0,trfs_record_struct);
                trfs_append_to_list(old_dentry, 1,trfs_record_struct);
		trfs_append_to_list(new_dentry, 1,trfs_record_struct);
		trfs_record_struct->success_flag = err;
                trfs_add_record(trfs_record_struct);
